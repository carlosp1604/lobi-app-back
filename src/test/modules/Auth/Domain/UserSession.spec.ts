...
describe('UserSession', () => {
  ...
  describe('create', () => {
    ...
    it('should initialize the UserSession instance correctly', () => {
      ...
      const session = UserSession.create(id, userId, tokenHash, deviceInfo, expiresTtlMs, now, userSessionIpHash, deviceLocation)
      ...
      expect(session.deviceInfo?.equals(deviceInfo)).toBe(true)
    ...
    it('should set to NULL when optional params are not given', () => {
      const session = UserSession.create(id, userId, tokenHash, deviceInfo, expiresTtlMs, now, null, null)
  ...
  describe('revoke', () => {
    ...
    it('should throw UserSessionDomainException when the session is already revoked', () => {
      ...
      expect(() => revokedSession.revoke(now)).toThrow(UserSessionDomainException.sessionAlreadyRevoked())
    })
    ...
    it('should throw UserSessionDomainException when the session is already expired', () => {
      ...
      expect(() => expiredSession.revoke(now)).toThrow(UserSessionDomainException.sessionAlreadyExpired())
    })
  })
  ...
  describe('canBeRevoked', () => {
    ...
    it('should return UserSessionDomainException error when session is already revoked', () => {
      ...
      expect(result['error']).toStrictEqual(UserSessionDomainException.sessionAlreadyRevoked())
    })
    ...
    it('should return UserSessionDomainException error when session is already expired', () => {
      ...
      expect(result.success).toBe(false)
      expect(result['error']).toStrictEqual(UserSessionDomainException.sessionAlreadyExpired())
    })
  })
