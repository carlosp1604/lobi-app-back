...
export class PostgreSqlUserCredentialRepository implements UserCredentialRepositoryInterface {
   ...
   */
  public async saveFailedAttempts(userCredential: UserCredential, context: TxContext): Promise<void> {
    ...
    const { user_id, failed_attempts, updated_at } = UserCredentialModelTranslator.toDatabase(userCredential)
   ...
   * @param context the transactional context
   */
  public async saveLock(userCredential: UserCredential, context: TxContext): Promise<void> {
    const entityManager = this.entityManagerResolver.resolve(context)

    const userCredentialRepository = entityManager.getRepository(UserCredentialEntity)

    const { user_id, locked_until, updated_at } = UserCredentialModelTranslator.toDatabase(userCredential)
